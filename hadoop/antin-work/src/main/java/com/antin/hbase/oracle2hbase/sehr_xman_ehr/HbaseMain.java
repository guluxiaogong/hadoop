...
public class HbaseMain {
    ...
    public static void main(String[] args) throws Exception {
        String tableName = "sehr_xman_ehr_0", hTableName = "sehr_xman_ehr", time = "2016-06-01 16:33:43";
        int maps = 1;
...
//        conf.set("hbase.zookeeper.property.clientPort", "2181");
        ...
        //根据时间增量
        DataDrivenDBInputFormat.setInput(job, SehrXmanEhrModel.class, "select * from (select rownum as id,xman_id,event,catalog_code,t.xml.getclobval() xml,commit_time from " + tableName + " t) b where commit_time>to_date('" + time + "','yyyy-MM-dd HH24:mi:ss') and $CONDITIONS", "select MIN(t.id), MAX(t.id) from (select rownum as id from " + tableName + " t where commit_time>to_date('" + time + "','yyyy-MM-dd HH24:mi:ss')) t");

        //DataDrivenDBInputFormat.setInput(job, SehrXmanEhrModel.class, "select * from (select rownum as id,xman_id,event,catalog_code,t.xml.getclobval() xml,commit_time from " + tableName + " t) b where $CONDITIONS", "select MIN(t.id), MAX(t.id) from (select rownum as id from " + tableName + ") t");
